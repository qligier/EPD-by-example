...
!!! danger
    ...
    error 500 for example). Failure to do so is a sufficient reason for EPR communities to block your access.
...
=== "First ActiveParticipant"
    ...
    - `user_id` is required and can be filled as you want. It can be a process ID, a login name, or other identifiers;
    - `alias` (optional) is the SAML Assertion's `#!xpath Subject/NameID/@SPProvidedID` attribute;
    - `user` (required) is the SAML Assertion's `#!xpath Subject/NameID` content;
    - `issuer` (required) is the SAML Assertion's `#!xpath Issuer` content.
    
    The other attributes and subject role specification are optional.

=== "Second ActiveParticipant"

    The **second additional `ActiveParticipant`** is described in the [Amendment 1 to Annex 5][annexes], §1.6.4.3.5.1.
    It is required for all roles.
    It shall contain:
    ```xml
    ...
    - `user_id` (required) is the SAML Assertion's `#!xpath Subject/NameID` content;
    - `user_name` (required) is the SAML Assertion's `A#!xpath ttributeStatement/Attribute[@Name="urn:oasis:names:tc:xspa:1.0:subject:subject-id"]/AttributeValue` content;
    - `code` (required) is the SAML Assertion's `#!xpath AttributeStatement/Attribute[@Name="urn:oasis:names:tc:xacml:2.0:subject:role"]/AttributeValue/Role/@code` attribute.

    Other attributes are optional.

=== "Third ActiveParticipant"

    The **third additional `ActiveParticipant`** is described in the [Amendment 1 to Annex 5][annexes], §1.6.4.3.5.1.
    ...
    - `user_id` (required) is the SAML Assertion's `#!xpath Subject/SubjectConfirmation/NameID` content;
    - `user_name` (required) is the SAML Assertion's `#!xpath Subject/SubjectConfirmation/SubjectConfirmationData/AttributeStatement/Attribute[@Name="urn:oasis:names:tc:xspa:1.0:subject:subject-id"]/AttributeValue` content;
...
- `transaction_code` and `transaction_display` are the same as in `#!xpath EventIdentification/EventID`;
...
Improve style for tabbed content.
